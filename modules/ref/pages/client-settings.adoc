...
To configure SDK client settings by writing code, provide a configuration when creating the `Cluster` instance. Here's an example that configures several settings by passing the optional third argument to `Cluster.Create()`:
...
var cluster = Cluster.Create(
    ...
    configureOptions: options => options // <1>
        .WithTimeoutOptions(timeoutOpts => timeoutOpts // <2>
            .WithQueryTimeout(TimeSpan.FromSeconds(15)))
        .WithSecurityOptions(securityOpts => securityOpts // <3>
            .WithTrustOnlyCapella())
);
----
<1> The parameter type is `Func<ClusterOptions, ClusterOptions>`.
The SDK passes a `ClusterOptions` object to your consumer.
Configure the options by calling methods on the given object.

<2> The `*Options` classes are immutable record types.
Every method returns the same record, allowing for method call chaining, but returns a new copy.

<3> Most users won't need to configure security settings, but it's shown here for completeness.
...
> A duration string is a possibly signed sequence of decimal numbers, each with optional fraction and a unit suffix, such as "300ms", "-1.5h" or "2h45m". Valid time units are "us", "ms", "s", "m", "h".

The SDK rejects non-positive timeout durations.
...
Consult your .NET SDK reference documentation for a list of supported cipher suite names.
...
The SDK uses a component called a `Deserializer` to convert query result rows into .NET objects. The default implementation is `StjJsonDeserializer`, a thin wrapper using System.Text.Json.

By default, a System.Text.Json `JsonSerializer` throws an exception if it encounters a JSON field that does not match the structure of the target class.
Here's an example that shows how to configure the `JsonSerializer` with different behavior:

[source,csharp]
----
var cluster = Cluster.Create(
    connectionString,
    Credential.Create(username, password),
...
TIP: If you prefer not to work with the `Deserializer` interface, you can always call `row.ContentAs<byte[]>()` to get a row's content as a raw byte array.
