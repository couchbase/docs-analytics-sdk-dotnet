...
====
The 1.1 {name-sdk} adds support for JWT and client certificate authentication, as well as a new poll-based {async-server-api-feature-name} that uses request handles to fetch results, introduced in self-managed Enterprise Analytics Server 2.2, and eliminating the need for long-running server connections.

The examples in this section are for the standard API, working with all 2.x releases of Enterprise Analytics (with {async-server-api-feature-name} examples following in the <<#server-asynchronous-api,Server Async section>>).
